...
(function($) {
    ...
    if ($('.tpd-select2').length > 0) {
        $(document).ready(function() {
            $('.tpd-select2').select2({
                placeholder: 'جستجوی دسته دوره سابق طراحی، توسعه، تجارت',
                allowClear: true,
                tags: true
            });
        });
    ...
    //     dateFormat: "Y-m-d", // فرمت تاریخ میلادی
    //     defaultDate: new Date(), // تنظیم تاریخ پیش‌فرض به امروز (میلادی)
    //     locale: {
    //         weekdays: {
    //             shorthand: ["ش", "ی", "د", "س", "چ", "پ", "ج"],
    //             longhand: ["شنبه", "یک‌شنبه", "دو‌شنبه", "سه‌شنبه", "چهار‌شنبه", "پنج‌شنبه", "جمعه"]
    //         },
    //         months: {
    //             shorthand: [
    ...
    //         const gregorianDate = selectedDates[0];
    ...
    //             // فرمت کردن تاریخ شمسی به صورت yyyy-mm-dd
    //             const formattedJalaaliDate = `${jalaaliDate.jy}-${jalaaliDate.jm.toString().padStart(2, '0')}-${jalaaliDate.jd.toString().padStart(2, '0')}`;
                
    //             // نمایش تاریخ شمسی
    //             if (document.querySelector("input.numInput.cur-year")) {
    //             this.textContent = formattedJalaaliDate;
    //             }

    //             // مقدار فیلد ورودی را تغییر دهید (اختیاری)
    //             instance.input.value = formattedJalaaliDate;
    //         }
    //     }
    // });
    $(document).ready(function() {
        $('#datetime-picker').persianDatepicker({
            format: 'YYYY/MM/DD', // فرمت تاریخ
            initialValue: false, // اگر می‌خواهید تاریخ پیش‌فرض داشته باشید
            onSelect: function(date) {
                // تاریخ انتخاب شده به فرمت شمسی
                var selectedDate = date; // تاریخ به فرمت YYYY/MM/DD
                document.getElementById("selected-date").textContent = selectedDate;
                $('#datetime-picker').val(selectedDate); // قرار دادن تاریخ در فیلد ورودی
            }
        });
    });
    ...
    if ($('#calendar').length > 0) {
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                slotMaxTime: '20:00',
                headerToolbar: {
                    left: 'dayGridMonth,timeGridWeek,timeGridDay', // نمایش دکمه‌ها در راست
                    center: 'title',
                    right: 'prev,next' // دکمه‌های جابجایی در راست
                },
                buttonText: {
                    dayGridMonth: 'ماه',
                    timeGridWeek: 'هفته',
                ...
                allDayText: 'تمام روز', // متن فارسی برای تمام روز
                initialView: 'dayGridMonth',
                initialDate: new Date(),
                navLinks: true, // قابلیت کلیک روی روزها/هفته‌ها برای تغییر نما
                editable: true,
                selectable: true,
                direction: 'rtl', // راست‌چین کردن کل ساختار
            });
    ...
    if ($('#chart').length > 0) {
        ...
        var options = {
            ...
            xaxis: {
                ...
                reversed: false, // به صورت پیشفرض، محورها به راست به چپ تبدیل می‌شوند
                labels: {
                    align: 'right'  // چیدمان برچسب‌ها به سمت راست
                }
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            showMoreButton.addEventListener('click', function() {
                ...
                if (content.classList.contains('show')) {
                    ...
                    showMoreButton.append('بارگزاری بیشتر');
                } else {
					...
					`;
                    showMoreButton.appendChild(svgIcon);
                    showMoreButton.append('بارگزاری کمتر');
                }
            });
        });
    }
